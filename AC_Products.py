...
class Product:
    ...
    def __init__(self, owning_Urn, type):
    ...
    def apply_ProductRule(self, product_rule):
...
class Urn:
    ...
    def __init__(self, urn_type, count_product_types, RNG,count_products=200):
        ...
        if urn_type == "fixed-rich":
            self.type = 1
            self.probability = (self.count_products/
                float(self.maxtype))/float(self.count_products)
            
        elif urn_type == "fixed-poor":
            self.type = 2
            
        elif urn_type == "endo-rich":
            self.type = 3
            self.collection = {}
            for i in range(self.maxtype):
                self.collection[i+1] = [Product(self,(i+1)) for j in range(
                    count_products)]
            
        elif urn_type == "endo-poor":
            self.type = 4
            self.collection = {1:[Product(self,1) for i in range(
                count_products)]}
    ...
    def request_product(self, desired_output, selective=False):
        ...
        if selective:
            ...
            if self.type == 1: # type = Rich
                return Product(self, desired_output)
